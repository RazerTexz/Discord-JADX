...
public class LinearCurveFit extends CurveFit {
    ...
    private double getLength2D(double d) {
        ...
        while (i2 < i) {
            ...
            if (d < dArr3[i3]) {
                ...
                double[][] dArr4 = this.f101mY;
                double d7 = dArr4[i2][0];
                double d8 = dArr4[i3][0];
                double d9 = 1.0d - d6;
                return Math.hypot(d5 - ((dArr4[i3][1] * d6) + (dArr4[i2][1] * d9)), d4 - ((d8 * d6) + (d7 * d9))) + dHypot;
            }
            i2 = i3;
            d2 = d4;
            d3 = d5;
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f100mT;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f101mY[0].length;
        if (d <= dArr2[0]) {
            for (int i2 = 0; i2 < length2; i2++) {
                dArr[i2] = this.f101mY[0][i2];
            }
            return;
        }
        int i3 = length - 1;
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (d <= dArr[0]) {
            for (int i2 = 0; i2 < length2; i2++) {
                fArr[i2] = (float) this.f101mY[0][i2];
            }
            return;
        }
        int i3 = length - 1;
        ...
        while (i4 < i3) {
            if (d == this.f100mT[i4]) {
                for (int i5 = 0; i5 < length2; i5++) {
                    fArr[i5] = (float) this.f101mY[i4][i5];
                }
            }
            double[] dArr2 = this.f100mT;
            int i6 = i4 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        int i2 = 0;
        if (d <= dArr[0]) {
            return this.f101mY[0][i];
        }
        int i3 = length - 1;
        if (d >= dArr[i3]) {
            return this.f101mY[i3][i];
        }
        while (i2 < i3) {
            double[] dArr2 = this.f100mT;
            if (d == dArr2[i2]) {
                return this.f101mY[i2][i];
            }
            int i4 = i2 + 1;
            if (d < dArr2[i4]) {
