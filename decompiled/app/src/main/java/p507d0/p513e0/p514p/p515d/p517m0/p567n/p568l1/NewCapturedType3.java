...
public final class NewCapturedType3 {
    ...
    */
    public static final KotlinType4 captureFromArguments(KotlinType4 kotlinType4, EnumC11973b enumC11973b) {
        ArrayList arrayList;
            ...
            if (!z2) {
                ...
                for (Tuples2 tuples2 : listZip) {
                    ...
                    if (typeProjectionAsTypeProjection.getProjectionKind() != variance) {
                        ...
                        typeProjectionAsTypeProjection = TypeUtils2.asTypeProjection(new NewCapturedType(enumC11973b, kotlinType3Unwrap, typeProjectionAsTypeProjection, typeParameterDescriptor));
                    }
                    arrayList2.add(typeProjectionAsTypeProjection);
                }
                TypeSubstitutor2 typeSubstitutor2BuildSubstitutor = TypeSubstitution3.f24889b.create(kotlinType4.getConstructor(), arrayList2).buildSubstitutor();
                ...
                if (size >= 0) {
                    while (true) {
                        ...
                        if (typeProjection.getProjectionKind() != variance) {
                            ...
                            ArrayList arrayListM840a0 = outline.m840a0(upperBounds, "type.constructor.parameters[index].upperBounds");
                            Iterator<T> it2 = upperBounds.iterator();
                            while (it2.hasNext()) {
                                arrayListM840a0.add(NewKotlinTypeChecker.f24820b.getDefault().transformToNewType(typeSubstitutor2BuildSubstitutor.safeSubstitute((KotlinType) it2.next(), variance).unwrap()));
                            }
                            if (!typeProjection.isStarProjection() && typeProjection.getProjectionKind() == Variance.OUT_VARIANCE) {
                                arrayListM840a0.add(NewKotlinTypeChecker.f24820b.getDefault().transformToNewType(typeProjection.getType().unwrap()));
                            }
                            ((NewCapturedType) typeProjection2.getType()).getConstructor().initializeSupertypes(arrayListM840a0);
                        }
                        if (i2 > size) {
                            break;
                        }
