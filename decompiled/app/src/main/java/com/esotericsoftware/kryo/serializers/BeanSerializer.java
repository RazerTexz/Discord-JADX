...
public class BeanSerializer<T> extends Serializer<T> {
    ...
    public BeanSerializer(Kryo kryo, Class cls) {
        try {
            ...
            Arrays.sort(propertyDescriptors, new C106221());
            ...
            try {
                this.access = MethodAccess.m924b(cls);
                int length = this.properties.length;
                for (int i = 0; i < length; i++) {
                    CachedProperty cachedProperty2 = this.properties[i];
                    cachedProperty2.getterAccessIndex = ((MethodAccess) this.access).m926c(cachedProperty2.getMethod.getName(), cachedProperty2.getMethod.getParameterTypes());
                    cachedProperty2.setterAccessIndex = ((MethodAccess) this.access).m926c(cachedProperty2.setMethod.getName(), cachedProperty2.setMethod.getParameterTypes());
                }
    ...
    public void write(Kryo kryo, Output output, T t) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                sb2.append(" (");
                throw new KryoException(outline.m867o(cls, sb2, ")"), e2);
            } catch (RuntimeException e3) {
                KryoException kryoException = new KryoException(e3);
                StringBuilder sb3 = new StringBuilder();
                sb3.append(cachedProperty);
                sb3.append(" (");
                outline.m864m0(cls, sb3, ")", kryoException);
                throw kryoException;
            } catch (InvocationTargetException e4) {
                ...
                sb4.append(" (");
                throw new KryoException(outline.m867o(cls, sb4, ")"), e4);
            }
        }
    }
}
