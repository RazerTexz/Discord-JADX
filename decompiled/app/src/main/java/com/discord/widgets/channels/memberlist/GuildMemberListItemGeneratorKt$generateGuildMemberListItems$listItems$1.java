    ...
    public final ChannelMembersListAdapter.Item invoke2(MemberListRow memberListRow) {
        ...
        if (memberListRow instanceof MemberListRow.StatusHeader) {
            ...
            if (memberListRow instanceof MemberListRow.Member) {
                ...
                boolean zAccess$calculateCanDisplayStatusEmoji = GuildMemberListItemGeneratorKt.access$calculateCanDisplayStatusEmoji(this.$storeGuilds, this.$storeCustomEmojis, member.getUserId(), this.$guild, this.$channel, member.getPresence(), this.$guildRoles);
