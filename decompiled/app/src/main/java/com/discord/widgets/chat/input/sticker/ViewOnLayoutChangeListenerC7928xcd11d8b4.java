...
public final class ViewOnLayoutChangeListenerC7928xcd11d8b4 implements View.OnLayoutChangeListener {
    ...
    public void onLayoutChange(View view, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
        ...
        RecyclerView recyclerView = WidgetStickerPicker.access$getBinding$p(this.this$0).f18198k;
        Intrinsics3.checkNotNullExpressionValue(recyclerView, "binding.chatInputStickerPickerRecycler");
        if (!ViewCompat.isLaidOut(recyclerView) || recyclerView.isLayoutRequested()) {
            recyclerView.addOnLayoutChangeListener(new ViewOnLayoutChangeListenerC7929x8893d084(this));
        } else {
