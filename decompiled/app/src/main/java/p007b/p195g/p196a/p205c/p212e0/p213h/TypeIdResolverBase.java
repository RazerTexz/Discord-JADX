package p007b.p195g.p196a.p205c.p212e0.p213h;

import p007b.p195g.p196a.p205c.JavaType;
import p007b.p195g.p196a.p205c.p212e0.TypeIdResolver;
import p007b.p195g.p196a.p205c.p218h0.TypeFactory;

/* JADX INFO: renamed from: b.g.a.c.e0.h.k, reason: use source file name */
/* JADX INFO: compiled from: TypeIdResolverBase.java */
/* JADX INFO: loaded from: classes3.dex */
public abstract class TypeIdResolverBase implements TypeIdResolver {

    /* JADX INFO: renamed from: a */
    public final TypeFactory f4774a;

    /* JADX INFO: renamed from: b */
    public final JavaType f4775b;

    public TypeIdResolverBase(JavaType javaType, TypeFactory typeFactory) {
        this.f4775b = javaType;
        this.f4774a = typeFactory;
    }

    @Override // p007b.p195g.p196a.p205c.p212e0.TypeIdResolver
    /* JADX INFO: renamed from: b */
    public void mo1947b(JavaType javaType) {
    }
}
