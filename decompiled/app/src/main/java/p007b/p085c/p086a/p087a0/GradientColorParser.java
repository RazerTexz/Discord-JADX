...
public class GradientColorParser implements ValueParser<GradientColor> {
    ...
    public GradientColor mo413a(JsonReader2 jsonReader2, float f) throws IOException {
        ...
        while (jsonReader2.mo607e()) {
            arrayList.add(Float.valueOf((float) jsonReader2.mo609n()));
        }
        if (z2) {
            jsonReader2.mo605c();
        }
        ...
        if (arrayList.size() > i) {
            ...
            while (true) {
                int[] iArr2 = gradientColor.f2745b;
                if (i2 >= iArr2.length) {
                    break;
                }
                int i10 = iArr2[i2];
                double d2 = gradientColor.f2744a[i2];
                int i11 = 1;
                while (true) {
                    if (i11 >= size) {
                        d = dArr2[size - 1];
                ...
                gradientColor.f2745b[i2] = Color.argb((int) (d * 255.0d), Color.red(i10), Color.green(i10), Color.blue(i10));
                i2++;
            }
        }
